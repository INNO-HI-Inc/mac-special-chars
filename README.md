...
## 3. 인감·서명 클립보드 — 선택 (개인정보, 로컬 전용)

계약서에 붙일 **인감(도장) 이미지**와 **자주 쓰는 텍스트**(이메일·주소·계좌 등)를
⌥ Space 팔레트 맨 위에서 클릭 한 번에 클립보드로 복사합니다.

- **인감·도장**: PNG 이미지를 클릭하면 **이미지 그대로** 클립보드에 복사돼요(한글·워드·PDF에 ⌘V).
  - 기본 폴더: `~/.hammerspoon/seals/` (없으면 `~/Desktop/인감 모음` 을 자동으로 찾음)
  - 다른 폴더를 쓰려면 `~/.hammerspoon/special_chars_seals_dir.txt` 에 경로 한 줄을 적으세요.
- **내 서명(텍스트)**: 웹 치트시트 상단 "내 서명"에서 추가·편집하고, "팔레트로 내보내기"로
  `special_chars_signatures.json` 을 저장해 `~/.hammerspoon/` 에 넣으면 팔레트에도 나타나요.

> 🔒 **인감·서명·계좌 같은 개인정보는 이 저장소에 절대 올라가지 않습니다.**
> 이미지는 로컬 폴더에서만 읽고, 서명 파일과 인감 폴더는 `.gitignore`로 제외돼요.
> `hammerspoon/signatures.example.json` 은 형식을 보여주는 예시(가짜 값)일 뿐입니다.
